...
The one real gotcha: do not fingerprint by execution ID. That would split repeated failures of the same scheduled job into unrelated groups. This example fingerprints by workload and job name, while retaining the execution ID in context for audit work.
